...
import {
  closest, events, vendorPrefix, limit,
  getElementMargin, provideDisplayName, omit
} from '../utils';
...
export default function sortableContainer(WrappedComponent, config = {withRef: false}) {
  ...
  return class extends Component {
    ...
    handleStart = (e) => {
      ...
      const node = findSortableElementNode(e.target);

      if (node == null) {
        return;
      }

      if (useDragHandle && findSortableHandleNode(e.target) == null) {
        return;
      }
      ...
      this.manager.setActive(node.sortableInfo);
    ...
    handlePress = (e) => {
      ...
      const {
        axis,
        getHelperDimensions, helperClass, hideSortableGhost,
        onSortStart, useWindowAsScrollContainer,
      } = this.props;
      ...
      this.helper = getHelperAsRenderedElement({node, element});
      ...
      this.helper.style.top = `${boundingClientRect.top - margin.top}px`;
      this.helper.style.left = `${boundingClientRect.left - margin.left}px`;
      ...
      this.maxTranslate = {};
      if (this.axis.x) {
        this.minTranslate.x = ((useWindowAsScrollContainer) ? 0 : containerBoundingRect.left) - boundingClientRect.left - (this.width / 2);
        this.maxTranslate.x = ((useWindowAsScrollContainer) ? this.contentWindow.innerWidth : containerBoundingRect.left + containerBoundingRect.width) - boundingClientRect.left - (this.width / 2);
      }
      if (this.axis.y) {
        this.minTranslate.y = ((useWindowAsScrollContainer) ? 0 : containerBoundingRect.top) - boundingClientRect.top - (this.height / 2);
        this.maxTranslate.y = ((useWindowAsScrollContainer) ? this.contentWindow.innerHeight : containerBoundingRect.top + containerBoundingRect.height) - boundingClientRect.top - (this.height / 2);
      }
...
function findSortableElementNode(element) {
  return closest(element, el => el.sortableInfo != null);
}

function findSortableHandleNode(element) {
  return closest(element, el => el.sortableHandle != null)
}
